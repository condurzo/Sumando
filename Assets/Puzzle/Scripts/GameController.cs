...
public class GameController : MonoBehaviour {
	...
	[HideInInspector] public Stack<Command> commands = new Stack<Command> ();
	[HideInInspector] public int fieldCells;

	private Vector3 nextCellScale = new Vector3 (0.5f, 0.5f, 0.5f);
	private List <HexagonController> cellsToSpawn = new List<HexagonController> ();
	private List<HexagonController> sameCells = new List<HexagonController> ();
	private Queue <HexagonController> nextCells = new Queue<HexagonController> ();
	private HexagonController[,] cells;
	private ButtonController [,] backField;

	[HideInInspector] public int [] availableValues;
	private float [] availableProbabilities;
	...
	public void InGameContinue (string key) {
		...
		if (cellsOnField == fieldCells)
		{
//			Invoke("GameManager.instance.GameOver", 1.6f);
			GameManager.instance.GameOver();
//			if (GameManager.InContinueForAds != null) {
//				GameManager.InContinueForAds.Invoke ();
//			}
		} 
	}
...
//						yield return new WaitWhile ((() => isCombining));
						while (isCombining) {
							yield return null;
						}
...
//						yield return new WaitForSeconds (1.6f);
	...
	IEnumerator CombiningCells (int i, int j) {
		...
		if (sameCells.Count >= identicCells) {
			...
			//GPGData.OnConnectCells (sameCells [0].CellValue);
	...
	public static IEnumerator ChangePos (int iFrom, int jFrom, int iTo, int jTo) {
		...
		if (freeWay.Count > 0) {
			...
			instance.StartCoroutine (instance.MovingCell (instance.cells [iFrom, jFrom].transform, freeWay, instance.cells [iFrom, jFrom]));
...
//		obj.TileShadow (true);
	...
	HexagonController PasteFromPool () {
		HexagonController cell;
		GameObject obj = Pool.cells.Dequeue ();
		cell = obj.GetComponent<HexagonController> ();
	...
	public void SecondChance () {
		...
		for (int n = 0; n < nearCells.Count; n++) {
			GameObject explo;
			if (n == 0) {
				explo = Instantiate (explosion, Vector2.zero, Quaternion.identity) as GameObject;
			} else {
				explo = Instantiate (explosion_simple, Vector2.zero, Quaternion.identity) as GameObject;
			}
			explo.transform.SetParent (holder, false);
