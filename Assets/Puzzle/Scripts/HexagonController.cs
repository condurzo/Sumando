...
//		if (val <= maxVal) {
			for (int q = 0; q < GameController.instance.values.Count; q++) {
				...
				if (val == GameController.instance.values [q]) {
					_valueCounter = q;
					break;
				}
			}
//		} else { 
//			for (int imageSkin = 1; imageSkin < skins.Length; imageSkin ++) {
//				int delta = (int) (Math.Pow(maxVal, imageSkin) * Math.Pow(2, imageSkin));
//				for (int q = 0; q < GameController.instance.values.Count; q++) {
//					if (val == GameController.instance.values [q] * delta) {
//						_valueCounter = q;
//						skin.gameObject.SetActive (true);
//						skin.sprite = skins [imageSkin - 1];
//						break;
//					}
//				}
//			}
...
//	public void TileShadow (bool active) {
//		tileImage.SetActive (active);
//	}
